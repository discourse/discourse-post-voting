...
let createAsUpvotesSetInRequest = false;

acceptance("Discourse Upvotes - composer", function (needs) {
  needs.user();
  needs.settings({ upvotes_enabled: true });

  needs.hooks.afterEach(() => {
    createAsUpvotesSetInRequest = false;
  });

  needs.pretender((server, helper) => {
    server.post("/posts", (request) => {
      if (parsePostData(request.requestBody).create_as_upvotes === "true") {
        createAsUpvotesSetInRequest = true;
      }
  ...
  test("Creating new topic with Q&A format", async function (assert) {
    ...
    await composerActions.selectKitSelectRowByName(
      I18n.t("composer.composer_actions.create_as_upvotes.label")
    );

    assert.strictEqual(
      query(".action-title").textContent.trim(),
      I18n.t("composer.create_upvotes.label"),
    ...
    await composerActions.expand();
    await composerActions.selectKitSelectRowByName(
      I18n.t("composer.composer_actions.remove_as_upvotes.label")
    );
  ...
  test("Creating new topic in category with Q&A create default", async function (assert) {
    Category.findById(2).set("create_as_upvotes_default", true);
