...
function upvotesEnabledTopicResponse() {
  topicResponse.post_stream.posts[0]["upvotes_vote_count"] = 0;
  topicResponse.post_stream.posts[0]["comments_count"] = 1;
  topicResponse.post_stream.posts[0]["upvotes_has_votes"] = false;
  ...
  topicResponse.post_stream.posts[1]["upvotes_vote_count"] = 2;
  topicResponse.post_stream.posts[1]["upvotes_has_votes"] = true;
  topicResponse.post_stream.posts[1]["comments_count"] = 6;
  topicResponse.post_stream.posts[1]["upvotes_user_voted_direction"] = "up";
  ...
  topicResponse["is_upvotes"] = true;

  return topicResponse;
}

function upvotesTopicListResponse() {
  // will link to OP
  topicList.topic_list.topics[0].is_upvotes = true;
  ...
  topicList.topic_list.topics[1].is_upvotes = true;
  ...
  topicList.topic_list.topics[3].is_upvotes = true;
...
function setupupvotes(needs) {
  needs.settings({
    upvotes_enabled: true,
    min_post_length: 5,
    upvotes_comment_max_raw_length: 50,
    upvotes_enable_likes_on_answers: false,
  });
  ...
  needs.pretender((server, helper) => {
    server.get("/t/280.json", (request) => {
      ...
      return helper.response(upvotesEnabledTopicResponse());
    });

    server.get("/upvotes/comments", () => {
    ...
    server.delete("/upvotes/comments", () => {
      return helper.response({});
    });

    server.put("/upvotes/comments", () => {
      return helper.response({
        id: 1,
        user_id: 12345,
        name: "Some Name",
    ...
    server.post("/upvotes/vote/comment", () => {
      return helper.response({});
    });

    server.delete("/upvotes/vote/comment", () => {
      return helper.response({});
    });

    server.get("/latest.json", () => {
      return helper.response(upvotesTopicListResponse());
    });
  });
}

acceptance("Discourse Upvotes - anon user", function (needs) {
  setupupvotes(needs);
  ...
  test("Viewing comments", async function (assert) {
    ...
    assert.strictEqual(
      queryAll("#post_1 .upvotes-comment").length,
      1,
      "displays the right number of comments for the first post"
    );

    assert.strictEqual(
      queryAll("#post_2 .upvotes-comment").length,
      5,
      "displays the right number of comments for the second post"
    );

    await click(".upvotes-comments-menu-show-more-link");
  ...
  test("adding a comment", async function (assert) {
    await visit("/t/280");
    await click(".upvotes-comment-add-link");
  ...
  test("voting a comment", async function (assert) {
    await visit("/t/280");
    await click("#post_2 #upvotes-comment-2 .upvotes-button-upvote");

    assert.ok(exists(".login-modal"), "displays the login modal");
  });
});

acceptance("Discourse Upvotes - logged in user", function (needs) {
  setupupvotes(needs);
  needs.user();
  ...
  test("non Q&A topics do not have Q&A specific class on body tag", async function (assert) {
    ...
    assert.notOk(
      !!document.querySelector("body.upvotes-topic"),
  ...
  test("Q&A topics has relevant copy on reply button", async function (assert) {
    ...
    assert.strictEqual(
      ...
      I18n.t("upvotes.topic.answer.label"),
  ...
  test("sorting post stream by activity and votes", async function (assert) {
    ...
    assert.ok(
      query(".upvotes-answers-headers-sort-votes[disabled=true]"),
    ...
    await click(".upvotes-answers-headers-sort-activity");
    ...
    assert.ok(
      !!document.querySelector("body.upvotes-topic-sort-by-activity"),
      "appends the right class to body when topic is filtered by activity"
    );

    assert.ok(
      query(".upvotes-answers-headers-sort-activity[disabled=true]"),
      "disabled sort by activity button"
    );

    await click(".upvotes-answers-headers-sort-votes");
  ...
  test("validations for comment length", async function (assert) {
    await visit("/t/280");
    await click("#post_1 .upvotes-comment-add-link");

    await fillIn(".upvotes-comment-composer-textarea", "a".repeat(4));

    assert.strictEqual(
      query(".upvotes-comment-composer-flash").textContent.trim(),
      I18n.t("upvotes.post.upvotes_comment.composer.too_short", { count: 5 }),
      "displays the right message about raw length when it is too short"
    );

    assert.ok(
      exists(".upvotes-comments-menu-composer-submit[disabled=true]"),
      "submit comment button is disabled"
    );

    await fillIn(".upvotes-comment-composer-textarea", "a".repeat(6));

    assert.strictEqual(
      query(".upvotes-comment-composer-flash").textContent.trim(),
      I18n.t("upvotes.post.upvotes_comment.composer.length_ok", { count: 44 }),
    ...
    await fillIn(".upvotes-comment-composer-textarea", "a".repeat(51));

    assert.strictEqual(
      query(".upvotes-comment-composer-flash").textContent.trim(),
      I18n.t("upvotes.post.upvotes_comment.composer.too_long", { count: 50 }),
  ...
  test("adding a comment", async function (assert) {
    ...
    await fillIn(".upvotes-comment-composer-textarea", "this is some comment");
    await click(".upvotes-comments-menu-composer-submit");
  ...
  test("adding a comment with keyboard shortcut", async function (assert) {
    ...
    await fillIn(".upvotes-comment-composer-textarea", "this is a new test comment");

    await triggerEvent(".upvotes-comments-menu-composer-submit", "keydown", {
      key: "Enter",
      ctrlKey: true,
    });
  ...
  test("editing a comment", async function (assert) {
    ...
    assert.strictEqual(
      query("#post_1 .upvotes-comment-cooked").textContent,
      "Test comment 1",
      "displays the right content for the given comment"
    );

    await click("#post_1 .upvotes-comment-actions-edit-link");
    ...
    assert.ok(
      exists(".upvotes-comment-editor-submit[disabled=true]"),
      "submit comment button is disabled"
    );

    await fillIn("#post_1 .upvotes-comment-editor-1 textarea", "editing this");

    assert.strictEqual(
      query(".upvotes-comment-composer-flash").textContent.trim(),
      I18n.t("upvotes.post.upvotes_comment.composer.length_ok", { count: 38 }),
    ...
    await click("#post_1 .upvotes-comment-editor-1 .upvotes-comment-editor-submit");
    ...
    assert.ok(
      !exists("#post_1 .upvotes-comment-editor-1"),
  ...
  test("deleting a comment", async function (assert) {
    ...
    await click("#post_1 .upvotes-comment-actions-delete-link");
    await click("a.btn-primary");

    assert.ok(
      exists("#post_1 #upvotes-comment-1.upvotes-comment-deleted"),
  ...
  test("deleting a comment after more comments have been loaded", async function (assert) {
    ...
    await click("#post_2 .upvotes-comments-menu-show-more-link");
    ...
    const comments = queryAll("#post_2 .upvotes-comment-actions-delete-link");

    await click(comments[comments.length - 1]);
    await click("a.btn-primary");

    assert.ok(
      !exists("#post_2 .upvotes-comments-menu-show-more-link"),
      "updates the comment count such that show more link is not displayed"
    );

    assert.ok(
      exists("#post_2 #upvotes-comment-7.upvotes-comment-deleted"),
      "adds the right class to deleted comment"
    );
  });
  ...
  test("vote count display", async function (assert) {
    ...
    assert.ok(
      !exists("#post_2 #upvotes-comment-2 .upvotes-comment-actions-vote-count"),
      "does not display element if vote count is zero"
    );

    assert.strictEqual(
      query("#post_2 #upvotes-comment-3 .upvotes-comment-actions-vote-count").textContent,
      "3",
  ...
  test("voting on a comment and removing vote", async function (assert) {
    ...
    assert.strictEqual(
      query("#post_2 #upvotes-comment-2 .upvotes-comment-actions-vote-count").textContent,
      "1",
  ...
  test("receiving user post voted message where current user removed their vote", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      type: "upvotes_post_voted",
      id: topicResponse.post_stream.posts[1].id,
      upvotes_vote_count: 0,
      upvotes_has_votes: false,
      upvotes_user_voted_id: 19,
    ...
    assert.strictEqual(
      query("#post_2 span.upvotes-post-toggle-voters").textContent,
      "0",
      "displays the right count"
    );

    assert.notOk(
      exists("#post_2 .upvotes-button-upvote.upvotes-button-voted"),
  ...
  test("receiving user post voted message where post no longer has votes", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      ...
      upvotes_vote_count: 0,
      upvotes_has_votes: false,
      upvotes_user_voted_id: 280,
      upvotes_user_voted_direction: "down",
    });
  ...
  test("receiving user post voted message where current user is not the one that voted", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      type: "upvotes_post_voted",
      id: topicResponse.post_stream.posts[1].id,
      upvotes_vote_count: 5,
      upvotes_has_votes: true,
      upvotes_user_voted_id: 123456,
      upvotes_user_voted_direction: "down",
    });
    ...
    assert.strictEqual(
      query("#post_2 .upvotes-post-toggle-voters").textContent,
      "5",
    ...
    assert.notOk(
      exists("#post_2 .upvotes-button-downvote.upvotes-button-voted"),
  ...
  test("receiving user post voted message where current user is the one that voted", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      ...
      upvotes_has_votes: true,
      upvotes_user_voted_id: 19,
      upvotes_user_voted_direction: "up",
    });
  ...
  test("receiving post commented message when comment has already been loaded", async function (assert) {
    ...
    assert.ok(
      !exists("#post_1 #upvotes-comment-5678"),
  ...
  test("receiving post commented message when there are no more comments to load ", async function (assert) {
    ...
    assert.ok(
      query("#post_1 #upvotes-comment-5678").textContent.includes(
        "Test comment ABC"
      ),
  ...
  test("receiving post commented message when there are more comments to load", async function (assert) {
    ...
    assert.ok(
      !exists("#post_2 #upvotes-comment-5678"),
  ...
  test("receiving post comment trashed message for a comment that has not been loaded ", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      type: "upvotes_post_comment_trashed",
      id: topicResponse.post_stream.posts[1].id,
      comments_count: 5,
      comment_id: 12345,
    });
  ...
  test("receiving post comment trashed message for a comment that has been loaded", async function (assert) {
    ...
    assert.ok(
      exists("#post_2 #upvotes-comment-2.upvotes-comment-deleted"),
  ...
  test("receiving post comment edited message for a comment that has been loaded", async function (assert) {
    ...
    publishToMessageBus("/topic/280", {
      type: "upvotes_post_comment_edited",
      id: topicResponse.post_stream.posts[0].id,
    ...
    assert.strictEqual(
      query("#post_1 #upvotes-comment-1 .upvotes-comment-cooked").textContent.trim(),
      "this is a new comment cooked",
      "it updates the content of the comment"
    );

    await click("#post_1 #upvotes-comment-1 .upvotes-comment-actions-edit-link");

    assert.strictEqual(
      query(
        "#post_1 #upvotes-comment-1 .upvotes-comment-composer textarea"
      ).textContent.trim(),
