...
describe Upvotes::TopicViewSerializerExtension do
  ...
  it 'should return correct values' do
    Upvotes::VoteManager.vote(topic_post, user)
    Upvotes::VoteManager.vote(answer, user)
    Upvotes::VoteManager.vote(answer, Fabricate(:user))
    ...
    expect(posts.first[:upvotes_user_voted_direction]).to eq(QuestionAnswerVote.directions[:up])
    expect(posts.first[:upvotes_has_votes]).to eq(true)
    expect(posts.first[:upvotes_vote_count]).to eq(1)
    expect(posts.first[:comments]).to eq([])
    expect(posts.first[:comments_count]).to eq(0)

    expect(posts.last[:id]).to eq(answer.id)
    expect(posts.last[:upvotes_user_voted_direction]).to eq(QuestionAnswerVote.directions[:up])
    expect(posts.last[:upvotes_has_votes]).to eq(true)
    expect(posts.last[:upvotes_vote_count]).to eq(2)
  ...
  it 'should not include dependent_attrs when plugin is disabled' do
    ...
    expect(payload[:upvotes_enabled]).to eq(nil)
