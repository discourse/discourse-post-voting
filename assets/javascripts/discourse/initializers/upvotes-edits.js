...
function initPlugin(api) {
  api.removePostMenuButton("reply", (attrs) => {
    return attrs.upvotes_has_votes !== undefined;
  });

  api.removePostMenuButton("like", (_attrs, _state, siteSetting) => {
    return (
      _attrs.upvotes_has_votes !== undefined &&
      _attrs.post_number !== 1 &&
      !siteSetting.upvotes_enable_likes_on_answers
    );
  });

  api.addPostMenuButton("answer", (attrs) => {
    if (
      attrs.upvotes_has_votes === undefined ||
      attrs.post_number !== 1 ||
      !attrs.canCreatePost
    ) {
      return;
    ...
    const args = {
      action: "replyToPost",
      title: "upvotes.topic.answer.help",
      icon: "reply",
    ...
    if (!attrs.mobileView) {
      args.label = "upvotes.topic.answer.label";
    }
  ...
  function customLastUnreadUrl(context) {
    if (context.is_upvotes && context.last_read_post_number) {
  ...
  api.reopenWidget("post", {
    ...
    _refreshController() {
      ...
      this._topicController().appEvents.trigger("upvotes-topic-updated");
    },
  ...
  api.decorateWidget("post-article:before", (helper) => {
    ...
    result.push(
      helper.h("div.upvotes-answers-header.small-action", [
        helper.h(
          "span.upvotes-answers-headers-count",
          I18n.t("upvotes.topic.answer_count", { count: answersCount })
        ),
        helper.h("span.upvotes-answers-headers-sort", [
          helper.h("span", I18n.t("upvotes.topic.sort_by")),
          helper.attach("button", {
            action: "orderByVotes",
            contents: I18n.t("upvotes.topic.votes"),
            disabled: topicController.filter !== ORDER_BY_ACTIVITY_FILTER,
            className: `upvotes-answers-headers-sort-votes ${
          ...
          helper.attach("button", {
            ...
            contents: I18n.t("upvotes.topic.activity"),
            disabled: topicController.filter === ORDER_BY_ACTIVITY_FILTER,
            className: `upvotes-answers-headers-sort-activity ${
              topicController.filter === ORDER_BY_ACTIVITY_FILTER
                ? "active"
                : ""
            }`,
  ...
  api.decorateWidget("post-menu:after", (helper) => {
    ...
    if (
      ...
      result.push(helper.attach("upvotes-comments", attrs));
    }
  ...
  api.decorateWidget("post-avatar:after", function (helper) {
    ...
    if (model.topic?.is_upvotes) {
      const upvotesPost = helper.attach("upvotes-post", {
        count: model.get("upvotes_vote_count"),
        post: model,
      });

      result.push(upvotesPost);
    }
