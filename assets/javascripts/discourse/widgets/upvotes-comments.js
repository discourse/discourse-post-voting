...
export default createWidget("upvotes-comments", {
  tagName: "div.upvotes-comments",
  buildKey: (attrs) => `upvotes-comments-${attrs.id}`,
  ...
  html(attrs) {
    ...
    if (postCommentsLength > 0) {
      for (let i = 0; i < postCommentsLength; i++) {
        result.push(this.attach("upvotes-comment", attrs.comments[i]));
      }
    }

    if (attrs.canCreatePost) {
      result.push(
        this.attach("upvotes-comments-menu", {
          id: attrs.id,
