...
module Upvotes
  class VotesController < ::ApplicationController
    ...
    before_action :ensure_upvotes_enabled, only: [:create, :destroy]

    def create
      ensure_can_vote(@post)

      if Upvotes::VoteManager.vote(@post, current_user, direction: vote_params[:direction])
    ...
    def create_comment_vote
      ...
      if Upvotes::VoteManager.vote(comment, current_user, direction: QuestionAnswerVote.directions[:up])
        render json: success_json
      else
        render json: failed_json, status: 422
      end
    ...
    def destroy
      if !Topic.upvotes(@post.topic, current_user).exists?
      ...
      if !Upvotes::VoteManager.can_undo(@post, current_user)
        msg = I18n.t('vote.error.undo_vote_action_window', count: SiteSetting.upvotes_undo_vote_action_window.to_i)
      ...
      if Upvotes::VoteManager.remove_vote(@post, current_user)
        render json: success_json
      else
        render json: failed_json, status: 422
      end
    ...
    def destroy_comment_vote
      ...
      if Upvotes::VoteManager.remove_vote(comment, current_user)
        render json: success_json
      else
        render json: failed_json, status: 422
      end
    ...
    def ensure_upvotes_enabled
      raise Discourse::InvalidAccess.new unless @post.is_upvotes_topic?
    end
    ...
    def ensure_can_vote(votable)
      ...
      if votable.user_id == current_user.id
        error_message = "post.upvotes.errors.self_voting_not_permitted"
        ...
        elsif !Upvotes::VoteManager.can_undo(votable, current_user)
          error_message = "vote.error.undo_vote_action_window"
          error_message_params = { count: SiteSetting.upvotes_undo_vote_action_window.to_i }
        end
